...
class Game:  
    ...
    def __init__(self):
        ...
        self._just_game_cols = int(cols) - conf.INFO_RIGHT
        ...
        if self._just_game_rows < conf.MIN_HEIGHT or self._just_game_cols < conf.MIN_WIDTH:
            print(
        ...
        self._screen = Screen(self._just_game_rows, self._just_game_cols,self._info_box_height)
    ...
    def init_new_life(self):
        ...
        self.game_paddle = PaddleClass(self._just_game_cols//2)
    ...
    def print_game_details(self):
        ...
        print(f"Number of powerups there is {cnt}")
    ...
    def generate_brick_coordinates(self):
        ...
        for i in range(15,self._just_game_rows-2):
            ...
            seq_in_row=0
            for j in range(5,self._just_game_cols-5,3):
                seq_in_row+=1
                ...
                self.bricks_list.append(decided_class(i,j,seq_in_row,(seq_in_row-i),color_code))
            logging.critical("\n")
    ...
    def handle_input(self):
        ...
        if self._input_stream.is_pending():
            ...
            if inp == 'a':
                paddle_new_details=self.game_paddle.change_x(-1, self._just_game_cols)
            ...
            if paddle_new_details is not None:
                for ball_obj in self.balls_list:
                    if ball_obj.is_stuck==True:
                         ball_obj.follow_paddle(paddle_new_details[0]+paddle_new_details[1]//2)
    ...
    def did_ball_collide_with_wall(self,prob_c,prob_r):
        ...
        if prob_c<0 or prob_c >= self._just_game_cols:
            return True
    ...
    def game_over_screen(self, msg):
        ...
        print(f"Your score is {self._score}".center(os.get_terminal_size().columns))
        sys.exit()
        pass
    ...
    def did_ball_collide_with_this_brick(self,prob_c,prob_r, brick_obj,ball_obj):
        ...
        if not (brick_obj.left_r==prob_r and (dc>=0 and dc<brick_obj.len_c)):
            return False
        # brick has to be vulnerable

        # Hit brick
        # if brick_obj.damage(self.ball_obj.is_boss):
        #     self._score+=brick_obj.score_bounty
        #     unlucky_neighbors=brick_obj.get_unlucky_friends()
        #     for sad_brick in unlucky_neighbors:
        #         self.destroy_this_brick



        
        if brick_obj.power_factor==0:
            self.destroy_this_brick(brick_obj)
            ...
            if True:
                ...
                if chosen_powerup==ThruBall:
                    ...
                    for already_there_powerup in self.curr_powerups_list:
                        if isinstance(already_there_powerup, ThruBall) and already_there_powerup.status=="active":
    ...
    def is_there(self, a, b, unlucky_arr):
        for i in unlucky_arr:
            if i[0]==a and i[1]==b:
                return True
        return False
    def hit_brick(self, brick_obj,is_forced):
        # Hit brick
        if brick_obj.isVisible and brick_obj.damage(is_forced):
            ...
            for sad_brick in self.bricks_list:
                ...
                #     #logging.info("voila")
                #     pass
                if self.is_there(sad_brick.left_r,sad_brick.seq_id,unlucky_neighbors):
    ...
    def did_ball_collide_with_bricks(self,prob_c,prob_r,ball_obj):
        ...
        for a_brick in bricks_checklist:
            ...
            #     continue
            if self.did_ball_collide_with_this_brick(prob_c,prob_r, a_brick,ball_obj):
    ...
    def play(self):
        ...
        while True:
            ...
            assert(self.game_paddle.len_c==self.game_paddle.ascii_repr.shape[1])
